...
class ContactForm {
    ...
    protected $settings_file = "settings.json";
        ...
        foreach($f_fields as $value){
            if ($value[1]==="text"){
              ...
              // echo '<input type="text" name="'.$value[0].'" placeholder="'.$value[0].'">';
               ...
               <div class="input-group input-group-sm mb-3">
               <span class="input-group-text" id="inputGroup-sizing-sm">'.$label.'</span>
               <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" name="'.$value[0].'"></textarea>
               </div>
               ';
            }
    ...
    public function create_web3form_bootstrap($key="external_key_to_be_here"){
        ...
        echo '<button type="submit" class="btn btn-primary m-3">Submit Form</button>';
        echo '</form>';
        echo '</div>';
    }
}
